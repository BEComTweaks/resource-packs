...
def val_command(menu,command):
    if type(command) == list:
        if command[0] == "back":
            ...
            with open(f"{cdir()}/jsons/packs/{command[1]}","r") as packs:
                packjson = loads(packs.read())
                topic = packjson["topic"]
            with open(f"{cdir()}/jsons/others/selected_packs.json","r") as selpacks:
                selpackjson = loads(selpacks.read())
    ...
    while choice == None:
        ...
        for i in range(len(os.listdir(f"{cdir()}/jsons/packs"))):
            with open(f'{cdir()}/jsons/packs/{os.listdir(f"{cdir()}/jsons/packs")[i]}',"r") as packjson:
                clmthing()
                loadpackjson = loads(packjson.read())
        ...
        clrprint(f"{i+2}. Show Selected Packs",end="",clr="red")
